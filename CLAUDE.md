...
- **`server/`** — Express API + SQLite (modul bawaan `node:sqlite`, tanpa native build).
  ...
  - `index.js` — semua route API (`/api/desa`, `/api/potensi/*`, `/api/ekosistem/*`, `/api/peta`, `/api/analisis/kuadran`, `/api/kabupaten/:nama/*`, `/api/dashboard/summary`, `/api/import/*`).
  ...
  - `lib/recommendKabupaten.js` — Analisis Kabupaten (AI): agregasi deterministik per kabupaten (distribusi status desa, tier BUM Desa sesuai Kepmendes 145/2022, status KDMP, sektor potensi terbanyak, daftar "desa prioritas" = potensi tinggi & skor rendah) lalu diberikan ke LLM untuk narasi + rekomendasi kebijakan. Daftar desa prioritas SELALU dihitung deterministik dari DB (bukan dikarang LLM) — expose lewat `buildKabupatenContext` (dipakai juga oleh endpoint ringkasan non-AI) dan `getRekomendasiKabupaten` (cache di tabel `rekomendasi_kabupaten`, pola sama seperti `rekomendasi_produk`).
  ...
  - `lib/scope.js` — `mergeScope`/`assertDesaAccess`/`assertKabupatenAccess`: penegakan RBAC per request, dipanggil di HAMPIR SEMUA route data di `index.js`. Kalau menambah route data baru, WAJIB panggil salah satu dari sini juga - jangan andalkan frontend saja untuk menyembunyikan data.
